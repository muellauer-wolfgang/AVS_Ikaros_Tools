...
using System.Text;
using Autofac;
using Abgleich_Ikaros_Export_Subito_Query.Startup;
...
namespace Abgleich_Ikaros_Export_Subito_Query
{
  internal class Program
  {
    ...
    public static Dictionary<string, Ikaros_Akt_DTO> _ikarosDict { get; set; } = new();
    public static List<Migrations_Report_DTO> _migrationsReportListe { get; set; } = new();  
    ...
    static void Main(string[] args)
    {
      ...
      Console.WriteLine("Abgleich Subito Query SubitoAnr, IkarosAnr mit Ikaros Gläubiger");
      Console.WriteLine($"BasePath: {_config.BasePath}");
      IXlsx_Reader xlsxReader = _container.Resolve<IXlsx_Reader>();
      ICsv_Reader_Writer csvReaderWriter = _container.Resolve<ICsv_Reader_Writer>();
      int cnt = 0;
      foreach (Ikaros_Akt_DTO ia in csvReaderWriter.Retrieve_Akte(
        Path.Combine(_config.BasePath, _config.Data_File_Ikaros_Export))) {
        cnt ++; 
        if (!_ikarosDict.ContainsKey(ia.IkarosAnr)) {
          _ikarosDict[ia.IkarosAnr] = ia;
        } else {
          if (!_ikarosDict[ia.IkarosAnr].Auftraggeber.Equals(ia.Auftraggeber)) {
            Console.WriteLine($"Akt; {ia.IkarosAnr} AG: {_ikarosDict[ia.IkarosAnr].Auftraggeber} != {ia.Auftraggeber} ");
          }
        }
      }
      Console.WriteLine($"Anzahl Gelesen {cnt}");
      Console.WriteLine($"Anzahl IKAORS-Akte {_ikarosDict.Count}");
      List<Migrations_Report_DTO> mergedList = new List<Migrations_Report_DTO>();
      cnt = 0;
      foreach(Subito_Akt_DTO sa in xlsxReader.Retrieve_Akte(
        Path.Combine(_config.BasePath, _config.Data_File_Subito_Query))) {
        cnt++;
        Migrations_Report_DTO mDTO = new Migrations_Report_DTO();
        mDTO.SubitoAnr = sa.SubitoAnr;
        mDTO.IkarosAnr = sa.IkarosAnr;
        if (_ikarosDict.ContainsKey(sa.IkarosAnr)) {
          mDTO.Auftraggeber = _ikarosDict[sa.IkarosAnr].Auftraggeber;
        } else {
          mDTO.Auftraggeber = "N.N.";
        }
        mergedList.Add(mDTO);
      }
      Console.WriteLine($"Anzahl Gelesen {cnt}");
      Console.WriteLine($"Size MergedList: {mergedList.Count}");
      csvReaderWriter.Create_Akte_File(
        Path.Combine(_config.BasePath, _config.Data_File_Migration_Report),
        mergedList );
...
} // end namespace Abgleich_Ikaros_Export_Subito_Query
